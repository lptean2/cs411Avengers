...
export const requestBasketSeriesData = (basketIds, regionId) => {
  return async (dispatch) => {
...
export const requestItemsSeriesData = (basketItems, regionId) => {
  return async (dispatch) => {
      ...
      basketItemEntries.forEach(([basketId, items]) => {
        items.forEach(item => {
          if (!itemIdSet.has(item.ItemID)) {
            ...
            dispatch(requestItemSeriesData(item.ItemID, regionId));
          }
        })
      });
    }
